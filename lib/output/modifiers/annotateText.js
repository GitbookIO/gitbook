...
function annotateText(entries, glossaryFilePath, $) {
    entries.forEach(function(entry) {
        ...
        var pattern = /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]/; // test chinese character
        var regParts = [ // match chinese character without `\b`
            pattern.test(name.substr(0, 1)) ? '(' : '\\b(',
            pregQuote(name),
            pattern.test(name.substr(-1, 1)) ? ')' : ')\\b',
        ]
        var searchRegex = new RegExp( regParts.join('') , 'gi' );
