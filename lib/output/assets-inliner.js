...
module.exports = function assetsInliner(Base) {
    ...
    AssetsInliner.prototype.onOutputImage = function(page, src) {
        ...
        .then(function(){
            if (!location.isDataURI(src)) return;

            return that.convertInlinePNG(src)
            .then(function(_asset) {
                src = '/' + _asset;
            });
    ...
    // Converts a data: png file into a .png file
    AssetsInliner.prototype.convertInlinePNG = function(src) {
        if (this.inlinePngs[src]) return Promise(this.inlinePngs[src]);
        ...
        return this.createNewFile(DEFAULT_ASSETS_FOLDER, hash + ext)
        .then(function(filename) {
            that.inlinePngs[src] = filename;

            that.log.debug.ln('converting asset', src);
            return imagesUtil.convertInlinePNG(src, that.resolve(filename))
            .thenResolve(filename);
        });
    };
