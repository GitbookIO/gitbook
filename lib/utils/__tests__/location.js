...
describe('LocationUtils', function() {
    ...
    describe('.relative', function() {
        it('should resolve to a relative path (same folder)', function() {
            expect(LocationUtils.relative('links/', 'links/test.md')).toBe(
                'test.md',
            );
        });

        it('should resolve to a relative path (parent folder)', function() {
            expect(LocationUtils.relative('links/', 'test.md')).toBe(
                '../test.md',
            );
        });
        ...
        it('should resolve to a relative path (child folder)', function() {
            expect(
                LocationUtils.relative('links/', 'links/hello/test.md'),
            ).toBe('hello/test.md');
        });
    });
    ...
    describe('.flatten', function() {
        it('should remove leading slash', function() {
            ...
            expect(LocationUtils.flatten('/hello/cool.md')).toBe(
                'hello/cool.md',
            );
        });
    ...
    describe('.toAbsolute', function() {
        it('should correctly transform as absolute', function() {
            expect(LocationUtils.toAbsolute('http://google.fr')).toBe(
                'http://google.fr',
            );
            expect(LocationUtils.toAbsolute('test.md', './', './')).toBe(
                'test.md',
            );
            expect(LocationUtils.toAbsolute('folder/test.md', './', './')).toBe(
                'folder/test.md',
            );
        });
        ...
        it('should correctly handle windows path', function() {
            expect(
                LocationUtils.toAbsolute('folder\\test.md', './', './'),
            ).toBe('folder/test.md');
        });
        ...
        it('should correctly handle absolute path', function() {
            ...
            expect(LocationUtils.toAbsolute('/test.md', 'test', 'test')).toBe(
                '../test.md',
            );
            expect(
                LocationUtils.toAbsolute('/sub/test.md', 'test', 'test'),
            ).toBe('../sub/test.md');
            expect(LocationUtils.toAbsolute('/test.png', 'folder', '')).toBe(
                'test.png',
            );
        });

        it('should correctly handle absolute path (windows)', function() {
            expect(LocationUtils.toAbsolute('\\test.png', 'folder', '')).toBe(
                'test.png',
            );
        });

        it('should resolve path starting by "/" in root directory', function() {
            expect(LocationUtils.toAbsolute('/test/hello.md', './', './')).toBe(
                'test/hello.md',
            );
        });
