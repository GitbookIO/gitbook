...
function normalizeHtml(src, options) {
    ...
    $('a').each(function() {
        ...
        if (links.isAnchor(href)) {
            ...
            // Support absolute internal links: for such links we only need to remove first slash
            var absolutePath = pathName.charAt(0) == '/' ? pathName.substr(1) : links.join(options.base, pathName);
