    ...
    .then(function() {
        return that.parseLangs()
            .then(function() {
                ...
                if (multilingual) that.log.info.ln("Parsing multilingual book, with", that.langs.length,
                    "languages");
                ...
                that.books = _.map(that.langs, function(lang) {
                    ...
                    return new Book(
                        path.join(that.root, lang.path), _.merge({}, that.context, {
...
Book.prototype.generate = function(generator) {
    ...
    .then(function() {
        if (that.isMultilingual()) {
            ...
            .then(function() {
                return _.reduce(ops.directories || [],
            ...
            .then(function() {
                ...
                return _.reduce(ops.content || [],
                    function(prev, file, i) {
                        return prev.then(function() {
            ...
            return book.generate(options.ebookFormat)
                .then(function(_options) {
                    var copyFile = function(lang) {
                        ...
                        if (lang) {
                            _outputFile = _outputFile.slice(0, -path.extname(_outputFile).length) + "_" + lang +
                                path.extname(_outputFile);
                            _tmpDir = path.join(_tmpDir, lang);
                        }

                        book.log.debug.ln("copy ebook to", _outputFile);
                        return fs.copy(path.join(_tmpDir, "index." + options.ebookFormat), _outputFile);
                    };
                    ...
                    return Q()
                        .then(function() {
                            if (book.isMultilingual()) {
                                return Q.all(_.map(book.langs, function(lang) {
...
Book.prototype.parsePlugins = function() {
    ...
    return that.plugins.load(that.options.plugins)
        .then(function() {
            if (_.size(that.plugins.failed) > 0) return Q.reject(new Error("Error loading plugins: " + that.plugins
                .failed.join(",") + ". Run 'gitbook install' to install plugins from NPM."));
...
Book.prototype.fileExists = function(filename) {
    return fs.exists(path.join(this.root, filename));
};
...
Book.prototype.contentPath = function(link) {
    if (path.basename(link, path.extname(link)) == "README" || link == this.readmeFile) {
...
Book.init = function(root) {
    ...
    var chaptersPaths = function(chapters) {
        return _.reduce(chapters || [], function(accu, chapter) {
            ...
            return accu.concat([o].concat(chaptersPaths(chapter.articles)));
        }, []);
    };
