...
- a9e6814: Add `leaveAgentFeedback` and `leaveUserFeedback` assistant tools, reporting documentation findings to the site's team through the same path as the MCP `sendFeedback` tool, and record an `agent_feedback` insights event on both surfaces.
...
- 39316fc: Give the prompt block a default "Copy prompt" action alongside "Open in Claude", "Open in Codex" and "Open in Cursor", and remember the visitor's last pick across every prompt block.
...
- 357af3f: Only advertise RSS feeds on pages with Updates blocks.
- 4452fd7: Fix page link titles showing the site section name instead of the target space title.
- 7f8ab7d: Serve Markdown responses as plain text to OpenAI's ChatGPT Search crawler.
...
- 05cff0a: Hide heading and expandable anchor links on pages whose layout disables anchors (landing pages).
- 6928a9b: Hide card fields that render no content, along with their title
- 3ae1524: Hide the Intercom launcher while the Assistant panel is open, so it no longer overlaps the message box.
...
- 581abe0: Announce pagination at the top of every part of a multi-part llms-full.txt, with absolute links to the previous and next parts.
...
- bdc2497: Missing pages with accented or non-Latin characters in the URL now return 404 instead of an endless redirect.
...
- e4638f1: Pass page context to webframes opened from integration modals.
...
- e94a508: Fix keyboard focus getting trapped inside the header search popover, allowing Tab/Shift+Tab to exit it as expected.
...
- afbc57c: Don't track a page view for 404s on internal `~gitbook` routes, so they no longer appear as broken URLs.
- 0f0cfde: Narrow a table or card grid to the reader's content selection: choosing an option in a tab or a select button now filters any select column that offers it. The table says what it has been narrowed to, and offers a Clear that drops the selection.
...
- 5725da1: Let a tab keep its content selection across a rename, by honouring an explicit slug set on the tab instead of always deriving one from its title.
...
- 8dd9444: Track Insights link clicks on the Download button of file blocks.
