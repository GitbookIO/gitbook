...
export function PageTags(props: { page: RevisionPageDocument; revision: Revision }) {
    ...
    return (
        // Returns a fragment so to allow wrapping each individual tag next to title, instead of all tags at once.
        <>
            {pageTags.map((tagRef) => {
                ...
                return <Tag key={resolved.slug} label={resolved.label} className="ml-1.5" />;
            })}
        </>
    );
}
