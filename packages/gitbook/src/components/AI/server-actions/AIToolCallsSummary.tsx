...
async function DescriptionForSearchToolCall(props: {
    ...
    return (
        <details className={tcls('-ml-5 group flex w-full flex-col', hasResults ? 'gap-2' : '')}>
            ...
            {hasResults ? (
                <div className="mt-1 max-h-0 overflow-y-auto circular-corners:rounded-2xl rounded-corners:rounded-lg border border-tint-subtle p-2 opacity-0 transition-all duration-500 transition-discrete group-open:max-h-96 group-open:opacity-11">
