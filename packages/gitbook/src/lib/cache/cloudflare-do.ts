...
export const cloudflareDOCache: CacheBackend = {
    ...
    async get(entry, options) {
        return trace(
            ...
            async (span) => {
                const stub = await getStub(entry.tag);
                if (!stub) {
                    return null;
                }

                return (await stub.get<CacheEntry>(entry.key)) ?? null;
            },
        );
    },
    async set(entry) {
        return trace(
            ...
            async () => {
                const stub = await getStub(entry.meta.tag);
                if (!stub) {
                    return;
                }

                await stub.set<CacheEntry>(entry.meta.key, entry, entry.meta.expiresAt);
            },
        );
    },
    async del(entries) {
