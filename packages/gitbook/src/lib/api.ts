...
function parseSiteSectionsList(siteSectionId: string, sectionsAndGroups: (SiteSectionGroup | SiteSection)[]) {
    const sections = sectionsAndGroups.flatMap(item => item.object === 'site-section-group' ? item.sections : item);
